...
class ProcessStructure:
    ...
    def get_block_list(
        self, ignored_blocks=["minecraft:air", "minecraft:structure_block"]
    ):
        block_counter = {}
        i = -2
    ...
    def __init__(self, file_list, exclude_list=[]):
        ...
        for file in file_list:
            ...
            self.structs[file]["blocks"] = array(
                list(
                    map(int, self.structs[file]["nbt"]["structure"]["block_indices"][0])
                )
            )

            self.structs[file]["size"] = array(
                list(map(int, self.structs[file]["nbt"]["size"]))
            )
            self.structs[file]["palette"] = self.structs[file]["nbt"]["structure"][
                "palette"
            ]["default"]["block_palette"]
            index_of_air = 0
            for i in range(len(self.structs[file]["palette"])):
                if self.structs[file]["palette"][i]["name"] == "minecraft:air":
                    index_of_air = i
            self.structs[file]["mins"] = array(
                list(map(int, self.structs[file]["nbt"]["structure_world_origin"]))
            )
            self.structs[file]["maxs"] = (
                self.structs[file]["mins"] + self.structs[file]["size"]
            )
            self.maxs = maximum(self.maxs, self.structs[file]["maxs"])
            self.mins = minimum(self.mins, self.structs[file]["mins"])
            self.structs[file]["blocks"] = self.structs[file]["blocks"].reshape(
                self.structs[file]["size"]
            )
            self.structs[file]["blocks"] = self.structs[file]["blocks"] + len(
                self.palette
            )
            self.structs[file]["blocks"][
                self.structs[file]["blocks"] == index_of_air + len(self.palette)
            ] = 0
        ...
        for file in file_list:
            embed(
                self.structs[file]["blocks"],
                self.blocks,
                self.structs[file]["mins"] - self.mins,
            )
...
if __name__ == "__main__":
    ...
    test = ProcessStructure(testFileName)
        ...
        for z in range(test.size[2]):
            ...
            if block["name"] not in excludedBlocks:
                ...
                if variant not in blocks_def[block["name"]].keys():
                    actual_name = input(
                        f"loc: {x + test.origin[0]},{z + test.origin[2]} {block['name']} - {variant}:"
                    )

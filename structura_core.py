...
class UnsupportedBlock:
    ...
    def __init__(
        self, pos: tuple[int, int, int], block: nbtlib.nbt.Compound, variant: str
    ):
        self.pos = pos
        self.block = block
    ...
    def __str__(self):
        return "x:{} Y:{} Z:{}, Block:{}, Variant: {}".format(
            self.pos[0], self.pos[1], self.pos[2], self.block["name"], self.variant
        )
    ...
    def __eq__(self, other):
        ...
        return (
            self.block["name"] == other.block["name"] and self.variant == other.variant
        )
...
class Structura:
    ...
    def make_big_model(self, offset):
        ...
        struct2make = structure_reader.CombinedStructures(
            file_names, exclude_list=self.exclude_list
        )
        self.structure_files[""] = {}
    ...
    def generate_with_nametags(self):
        for model_name in self.structure_files.keys():
            ...
            # temp folder would be a good idea
            copyfile(
                self.structure_files[model_name]["file"],
                "{}/{}.mcstructure".format(self.pack_name, model_name),
            )
            logger.debug(self.structure_files[model_name]["offsets"])
            struct2make = structure_reader.ProcessStructure(
                self.structure_files[model_name]["file"]
            )
            ...
            self.armorstand_entity.export(
                self.pack_name
            )  # this may be in the wrong spot, but transferred from 1.5
    ...
    def make_nametag_block_lists(self):
        ...
        for model_name in self.structure_files.keys():
            ...
            with open(file_name, "w+") as text_file:
                text_file.write(
                    "This is a list of blocks, there is a known issue with variants, all blocks are reported as minecraft stores them\n"
                )
    ...
    def _add_blocks_to_geo(self, struct2make, model_name, export_big=False):
        ...
        armorstand = asgc.armorstandgeo(
            model_name,
            alpha=self.opacity,
            size=[xlen, ylen, zlen],
            offsets=self.structure_files[model_name]["offsets"],
        )
            ...
            for loc in non_air:
                ...
                if debug:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Unsupported block '{}' in this world and it was skipped".format(
                                blk_name
                            )
                        )
    ...
    def compile_pack(self, overwrite=False):
        ...
        larger_render_path = (
            f"{self.pack_name}/models/entity/armor_stand.larger_render.geo.json"
        )
    ...
    def _process_block(self, block):
        ...
        data = 0
        variant = "default"
        for key in self.nbt_def.keys():
            if self.nbt_def[key] == "variant" and key in block["states"].keys():
                variant = [key, block["states"][key]]
            if self.nbt_def[key] == "rot" and key in block["states"].keys():
                try:
                    rot = int(block["states"][key])
                except Exception:
                    rot = str(block["states"][key])

            if self.nbt_def[key] == "top" and key in block["states"].keys():
                top = bool(block["states"][key])
            if self.nbt_def[key] == "open_bit" and "open_bit" in block["states"].keys():
                open_bit = bool(block["states"][key])
            if self.nbt_def[key] == "data" and key in block["states"].keys():

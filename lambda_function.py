...
def tempLambda(event, context):
    ...
    if "token" in event["headers"].keys():
        try:
            ...
            if decoded["auth"]:
                try:
                    ...
                    data = {
                        "content": "failed due to error processing file. Error {}, in file {}, line number {} ".format(
                            str(e), fname, exc_tb.tb_lineno
                        )
                    }
        ...
        try:
            ...
            try:
                verify_key.verify(
                    f"{timestamp}{body2}".encode(), bytes.fromhex(signature)
                )
            ...
            if body["channel"]["id"] in channel:
                ...
                data = {
                    "content": f"Converstion in this channel are disallowed. Please use <#{channelpref}> to convert files",
                    "flags": 64,
                }
...
def makeStructuraLabPack(headder, userInfo):
    ...
    if userInfo["username"] == itemData["Creator"]:
        ...
        response = s3_client.list_objects_v2(
            Bucket="structuralab.com", Prefix=f"{guid}/"
        )
        filesToConvert = {}
        try:
            ...
            for object in response["Contents"]:
                if object["Key"].endswith(".mcstructure"):
                    ...
                    s3_client.download_file(
                        "structuralab.com", object["Key"], filesToConvert[fileName]
                    )
        ...
        itemData["StructuraFile"] = (
            f"https://s3.us-east-2.amazonaws.com/structuralab.com/{guid}/{name}.mcpack"
        )
...
def errorResponse(errorCode, event):
    ...
    resp = {"statusCode": errorCode, "headers": corsHeaders, "body": json.dumps(event)}
    return resp
...
def add_command(body):
    headers = {"Authorization": "Bot {}".format(discord_secret)}
...
def initial_callback(body, ephemeral=False):
    data = {"type": 5, "data": {"flags": 0}}
    if ephemeral:
        data["data"]["flags"] = 64
    interaction_id = body["id"]
    interaction_token = body["token"]
    url = "https://discord.com/api/v10/interactions/{}/{}/callback".format(
...
def send_url_buttons(body, labels, urls, text="pack creation complete"):
    interaction_id = body["id"]
    interaction_token = body["token"]
    data = {"content": text, "components": [{"type": 1, "components": []}]}
    for i in range(len(labels)):
        button = {"type": 2, "label": labels[i], "style": 5, "url": urls[i]}
...
def help_command(body):
    ...
    pack_creation_time = float(response["Item"]["runTime"]) / float(
        response["Item"]["packsCreated"]
    )
...
def stats_command(body):
    ...
    help_text += (
        f"Broken Blocks                 \t: {in_github} reported,\t {seen} seen\n"
    )
...
def convert_command(body, tick):
    ...
    try:
        ...
        data = {"content": "working on conversion", "flags": 0}
        valid_files = []
        ...
        if len(valid_files) > 1:
            ...
            raise Exception(
                "No Valid Files were attached, Either File size is 0kb or no .mcstructure files were loaded"
            )
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
...
def make_pack_nametag(valid_files, body, tick):
    ...
    data["content"] = (
        "Processing, if this hangs it is because the file is too big. retrying will not fix it"
    )
    send_repsonse(body, data)
    ...
    pack_creation_time = float(stats["historicalTotal"]["runTime"]) / float(
        stats["historicalTotal"]["packsCreated"]
    )
...
def pack_per_youtube_View(creation_time):
    ...
    # cpm=5.97
    packs_per_view = cpm / (
        (creation_time * ram_allocated_GB * cost_compute + 4 * price_per_write_unit)
        * 1000
    )

...
class Animations:
        ...
        self.default_size = {
            ...
            "animations": {
                "animation.armor_stand.ghost_blocks.scale": {
                    "loop": True,
                    "bones": {"ghost_blocks": {"scale": 16.0}},
                }
            },
        }
        pathtofile = "{}/animations/armor_stand.animation.json".format(path_to_default)
    ...
    def insert_layer(self, y):
        ...
        for i in range(12):
            if y % (12) != i:
                self.sizing["animations"][self.poses[i + 1]]["bones"][name] = {
                    "scale": 0.08
                }
    ...
    def export(self, pack_name):
        ...
        path_to_rc = (
            "{}/animations/armor_stand.ghost_blocks.scale.animation.json".format(
                pack_name
            )
        )
        try:
    ...
    def export_big(self, pack_name, offset):
        print(offset)
        self.default_size["animations"]["animation.armor_stand.ghost_blocks.scale"][
            "bones"
        ]["ghost_blocks"]["rotation"] = [0, "-query.body_y_rotation", 0]
        self.default_size["animations"]["animation.armor_stand.ghost_blocks.scale"][
            "bones"
        ]["ghost_blocks"]["position"] = [

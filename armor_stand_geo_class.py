...
class armorstandgeo:
    def __init__(
        ...
        offsets=[0, 0, 0],
        size=[64, 64, 64],
        ref_pack="Vanilla_Resource_Pack",
    ):
        ...
        with open(
            "{}/textures/terrain_texture.json".format(self.ref_resource_pack)
        ) as f:
    ...
    def export(self, pack_folder):
        ...
        self.stand["minecraft:geometry"] = [
            self.geometry
        ]  # this is insuring the geometries are imported, there is an implied reference other places.
        ...
        for index in range(len(self.stand["minecraft:geometry"][0]["bones"])):
            if "name" not in self.stand["minecraft:geometry"][0]["bones"][index].keys():
                self.stand["minecraft:geometry"][0]["bones"][index]["name"] = (
                    "empty_row+{}".format(i)
                )
                self.stand["minecraft:geometry"][0]["bones"][index][
                    "parent"
                ] = "ghost_blocks"
                self.stand["minecraft:geometry"][0]["bones"][index]["pivot"] = [
                    0.5,
                    0.5,
                    0.5,
                ]
    ...
    def export_big(self, pack_folder):
        ...
        geometries = defaultdict(dict, dict, dict)
        geometries["default"] = {}
        geometries["default"]["description"] = {}
        geometries["default"]["description"][
            "identifier"
        ] = "geometry.armor_stand.default"
        ...
        geometries["default"]["bones"] = [
            {"name": "ghost_blocks", "pivot": [-8, 0, 8], "origin": [0, 0, 0]}
        ]
        default_geo = [
            {
                "size": size,
        ...
        for i in range(len(self.layers)):
            ...
            geometries[layer_name]["description"]["identifier"] = (
                "geometry.armor_stand.ghost_blocks_{}".format(i)
            )
            geometries[layer_name]["description"]["texture_width"] = 1
            geometries[layer_name]["description"]["texture_height"] = len(
                self.uv_map.keys()
            )
            ...
            geometries[layer_name]["bones"] = [
                {
                    "name": "ghost_blocks",
                    "pivot": [0, 0, 0],
                },  # i am not sure this should be this value for pivot
    ...
    def make_layer(self, y):
        ...
        self.geometry["bones"].append(
            {"name": layer_name, "parent": "ghost_blocks"}
        )  # , "pivot": [-8, 0, 8]})
    ...
    def make_block(
        ...
        if block_type != "ignore":
            ...
            self.blocks[ghost_block_name]["pivot"] = [
                block_shapes["center"][0] - (x + self.offsets[0]),
            ...
            if block_type in self.block_rotations.keys() and rot is not None:
                self.blocks[ghost_block_name]["rotation"] = copy.deepcopy(
                    self.block_rotations[block_type][str(rot)]
                )
                if big:
            ...
            for i in range(len(block_shapes["size"])):
                uv = self.block_name_to_uv(
                    block_name, variant=variant, shape_variant=shape_variant, index=i
                )
                block = {}
    ...
    def stand_init(self):
        ...
        self.geometry["description"]["visible_bounds_offset"] = [0.0, 1.5, 0.0]
    ...
    def block_name_to_uv(
        self, block_name, variant="", shape_variant="default", index=0, data=0
    ):

        # helper function maps the the section of the uv file to the side of the block
        temp_uv = {}
        if (
            block_name not in self.excluded
        ):  # if you dont want a block to be rendered, exclude the UV
            ...
            if shape_variant in self.block_uv[block_type].keys():
                if "overwrite" in self.block_uv[block_type][shape_variant].keys():
                    corrected_textures = self.block_uv[block_type][shape_variant][
                        "overwrite"
                    ]
            else:
                if "overwrite" in self.block_uv[block_type]["default"].keys():
                    corrected_textures = self.block_uv[block_type]["default"][
                        "overwrite"
                    ]
                ...
                if texture_files[key] not in self.uv_map.keys():
                    try:
                        self.extend_uv_image(
                            "{}/{}.png".format(
                                self.ref_resource_pack, texture_files[key]
                            )
                        )
                        self.uv_map[texture_files[key]] = len(self.uv_map.keys())
                    except Exception:
                        raise RuntimeError(
                            "Failed to load texture {}".format(texture_files[key])
                        )
                temp_uv[key] = {
                    "uv": [0, self.uv_map[texture_files[key]]],
                    "uv_size": [1, 1],
                }
